...
5. **Figure captions** (`AUTOMATION.FIGURE_*`, `ABOUT.FIGURE_*`, `MAINTENANCE.GLYPH_*`), their outcome lines (`*_OUTCOME`) and the nine section framing sentences (`GLOBAL.<SECTION>_INTRO`) in af, fr, sw and zu were drafted without a native speaker; include them in the review above.
